...
def get_oauth_credentials(project_id: Optional[str] = None) -> Dict[str, str]:
...
def _get_from_secret_manager(project_id: Optional[str]) -> Dict[str, str]:
...
def _get_from_env_or_file() -> Dict[str, str]:
    ...
    if not all([result['access_token'], result['folder_id']]):
        ...
        ifp_env_path = os.path.join(
            os.path.dirname(os.path.dirname(__file__)), 'ifp.env'
        )

        if os.path.exists(ifp_env_path):
            try:
                with open(ifp_env_path, 'r', encoding='utf-8') as f:
                    for line in f:
                        line = line.strip()
                        ...
                        if '=' in line:
                            ...
                            elif key == 'GOOGLE_DRIVE_CLIENT_SECRET' and not result['client_secret']:
                                result['client_secret'] = value
                            elif key == 'GOOGLE_DRIVE_FOLDER_ID' and not result['folder_id']:
                                result['folder_id'] = value
                            elif key == 'GOOGLE_DRIVE_REFRESH_TOKEN' and not result['refresh_token']:
...
def test_secret_manager(project_id: Optional[str] = None) -> bool:
