...
def print_report(verbose: bool = False) -> bool:
    ...
    if verbose:
        print(f"\n📋 來源清單：")
        for source, details in [("devcontainer", info["devcontainer"]),
                               ("vscode", info["vscode"]),
                               ("idx", info["idx"])]:
            print(f"\n  {source.upper()}")
...
def main():
    ...
    parser.add_argument("--fix", action="store_true", help="嘗試自動修復（覆寫 devcontainer 與 idx，以 .vscode/extensions.json 為準）")
    ...
    if args.fix:
        ...
        try:
            with open(DEVCONTAINER_JSON, 'r', encoding='utf-8') as f:
                dev_obj = json.load(f)
            dev_obj.setdefault("customizations", {}).setdefault("vscode", {})["extensions"] = sorted(vscode_exts)
        ...
        try:
            with open(IDX_DEV_NIX, 'r', encoding='utf-8') as f:
                nix_content = f.read()
