...
def run_metric_tree_diagnostic(report_summary: Dict[str, Any], thresholds: Thresholds | None = None) -> Dict[str, Any]:
    ...
    cvr_purchase_per_click_pct = (_safe_div(purchases_primary * 100.0, link_clicks)) if link_clicks > 0 else None
    ...
    if roas_primary < thresholds.roas_trigger:
        metric_tree_notes.append(f"Level1：ROAS={roas_primary:.4f} 未達目標 {thresholds.roas_trigger}。")
    else:
        metric_tree_notes.append(f"Level1：ROAS={roas_primary:.4f} 達標（目標 {thresholds.roas_trigger}）。")

    if cpa_primary is not None and aov_primary is not None:
        metric_tree_notes.append(f"Level2：ROAS ≈ AOV/CPA；AOV≈{aov_primary:.2f}、CPA≈{cpa_primary:.2f}（basis={roas_basis}）。")
    else:
        metric_tree_notes.append("Level2：缺少 purchases/value，無法計算 AOV/CPA（可能是追蹤異常或資料缺失）。")

    cpm_note = f"{cpm_twd:.2f}" if cpm_twd is not None else "未提供"
    ctr_note = f"{ctr_link_pct:.2f}%" if ctr_link_pct is not None else "未提供"
    cvr_note = f"{cvr_purchase_per_click_pct:.2f}%" if cvr_purchase_per_click_pct is not None else "未提供"
    metric_tree_notes.append(f"Level3：CPA ≈ CPM / (CTR×CVR)。CPM≈{cpm_note}、CTR≈{ctr_note}、CVR≈{cvr_note}。")

    if ctr_link_pct is not None and ctr_link_pct > 0 and ctr_link_pct < thresholds.ctr_link_pct_min:
        suspected.append(f"CTR 偏低（{ctr_link_pct:.2f}% < {thresholds.ctr_link_pct_min}%）：偏向『素材/受眾訊息』問題")
    if cvr_purchase_per_click_pct is not None and cvr_purchase_per_click_pct > 0 and cvr_purchase_per_click_pct < thresholds.cvr_purchase_per_click_min:
        suspected.append(f"CVR 偏低（{cvr_purchase_per_click_pct:.2f}% < {thresholds.cvr_purchase_per_click_min}%）：偏向『落地頁/產品/結帳』問題")
    ...
    if rates["purchase_per_ic"] is not None and rates["purchase_per_ic"] < thresholds.purchase_per_ic_min:
    ...
    recs: list[str] = []
    if purchase_value_truth == 0.0 and purchase_value_platform > 0.0:
        recs.append("先修復 Pixel/CAPI 的 Purchase value/currency 與去重，否則 ROAS（truth）全面失真。")
    recs.append("指標樹順序：先看 ROAS → 拆 AOV/CPA → 再拆 CPM/CTR/CVR → 最後用漏斗事件定位（LPV/ATC/IC/Purchase）。")
    if rates["lpv_per_click"] is None:
        recs.append("缺少 LPV/Clicks 無法判斷點擊→到站，請確認匯出含「連結頁面瀏覽次數」「連結點擊次數」。")
    if rates["purchase_per_ic"] is None:
        recs.append("缺少 initiate_checkout 或 purchases，無法定位結帳段問題，請確認匯出含「開始結帳次數」「購買次數」。")
    recs.append(f"若主 ROAS（{roas_basis}）低於 {thresholds.roas_trigger}，依序檢查：LPV/Click → ATC/LPV → IC/ATC → Purchase/IC。")
    recs.append("把最弱的一段做成 1~2 個最小改動 A/B（例如商品頁信任元素、運費/到貨資訊、結帳流程）。")
    ...
    return {
        ...
        "level3_decomposition": {
            ...
            "cvr_purchase_per_click_pct_calc": (round(cvr_purchase_per_click_pct, 4) if cvr_purchase_per_click_pct is not None else None),
        },
        "funnel_counts": {
