...
def test_start_with_pty(tmp_path):
    ...
    s = subprocess.run([SERVICE_SCRIPT, "status", "dummy_pty"], cwd=ROOT, capture_output=True, text=True)
    assert s.returncode == 0, f"status failed: {s.stderr}"

    # Stop and cleanup
    stop = subprocess.run([SERVICE_SCRIPT, "stop", "dummy_pty"], cwd=ROOT, capture_output=True, text=True)
...
def test_auto_fallback_to_pty(tmp_path):
    ...
    p = subprocess.run([SERVICE_SCRIPT, "start", "tty_auto", "--cmd", tt_cmd], cwd=ROOT, capture_output=True, text=True)
    assert p.returncode == 0, f"start failed: {p.stderr}"

    # Allow time for fallback logic and startup
    time.sleep(0.6)

    s = subprocess.run([SERVICE_SCRIPT, "status", "tty_auto"], cwd=ROOT, capture_output=True, text=True)
    ...
    # Stop and cleanup
    stop = subprocess.run([SERVICE_SCRIPT, "stop", "tty_auto"], cwd=ROOT, capture_output=True, text=True)
    assert stop.returncode == 0, f"stop failed: {stop.stderr}"
